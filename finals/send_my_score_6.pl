...
chomp($HOST=`hostname`);
open(LOG, "> /tmp/score-6_$TIME.txt") || die("Cannot open /tmp/score-6_$TIME.txt\n");
...
sub run_hbase
{
  print "[34;1m === <EHC> Run Hbase Performance Evaluation  === [0m\n";
  print " Running 'ssh -i ~/.vagrant.d/insecure_private_key vagrant@\"192.168.90.12\" \"hbase org.apache.hadoop.hbase.PerformanceEvaluation randomWrite 1\"'\n [32;1m Messages will show later ... Please wait ....\n[0m";
  chomp($OUTPUT=`ssh -i ~/.vagrant.d/insecure_private_key vagrant@"192.168.90.12" "hbase org.apache.hadoop.hbase.PerformanceEvaluation randomWrite 1 2>&1"`);

  print "$OUTPUT";
  print LOG "$OUTPUT";

  chomp($IO=`grep "Finished class org.apache.hadoop.hbase.PerformanceEvaluation" /tmp/score-6_$TIME.txt | awk '{ print \$9 }'`);
  print "\n\n [33;1m HBase = $IO ms\n[0m";
  $HBASE = $IO;
